...
def get_pie_chart(entered_site):
    ...
    #print(filtered_df)
    #return px.pie(filtered_df, values='class',labels = 'Launch Site', title = 'Pie Chart')
    if entered_site == 'ALL':
        #return px.pie(filtered_df, values='class',labels = 'Launch Site', title = 'Pie Chart')
        return px.pie(filtered_df, values='class', names = 'Launch Site', title = 'Pie Chart')
        #return px.pie(filtered_df,  names = 'class')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df = filtered_df.groupby('class').size().reset_index()
        fig = px.pie(filtered_df, values = filtered_df[0], names = filtered_df['class'], title = 'Pie Chart' )
        return fig
# TASK 4:
